...
/* //linpsk original
  ...
  */
  ...
  int pixelwidth=RX_MAX_COLUMNS*this_font.pointSize();
  //rowHeight=this_font.pointSize()*15/10;
  rowHeight=this_font.pointSize()*21/10;
  //end of hb9eid modification
  ...
  for ( int i = 0;i < RXWINDOWBUFFER;i++ )
  {
    ...
    ScrollBuffer[i]->setFont(this_font);//hb9eid
    //ScrollBuffer[i]->setMaximumHeight(18);
    ScrollBuffer[i]->setMaximumHeight(2*this_font.pointSize());
    //ScrollBuffer[i]->setFixedHeight(this_font.pointSize()*14/10);
