...
{
    //gpio initialsiation
    int initcode=gpioInitialise();
    if (initcode < 0)
    {
   // gpioinitialisation failed.
      char outline[100];
      sprintf(outline,"gpioInitialise failed: %d %d %s",initcode,__LINE__,__FUNCTION__);
      qDebug() << outline;
      QMessageBox::warning(this, "warning", "gpioInitialise() not open. Sudo forgotten?");
...
void LinPSK::closeEvent ( QCloseEvent *e )
{
  ...
  stop_dds();
  char outline[100];
  sprintf(outline,"end linpsk window %d %s",__LINE__,__FUNCTION__);
  qDebug() << outline;
  return;
}
